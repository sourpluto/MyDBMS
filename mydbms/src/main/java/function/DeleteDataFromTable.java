...
public class DeleteDataFromTable {
    ...
    public static void deleteFromTable(String dbName,String tbName,List<String> tmp) throws DocumentException, IOException {
        ...
        //获取where列名称
        String [] key_value=tmp.get(0).trim().split("\\s*=\\s*");
    ...
    public static boolean delete(File file,String dbName,String tbName,String[] key_value,String last_num) throws DocumentException, IOException {
        ...
        for (Node node : nodes) {
            ...
            for (Iterator i = lists.iterator(); i.hasNext(); ) {
                ...
                // 添加调试信息
                System.out.println("检查属性: " + attribute.getName() + " = " + attribute.getText());
                System.out.println("查找条件: " + key_value[0].trim() + " = " + key_value[1].trim());
                
                // 修复匹配逻辑，确保去除空格并进行字符串比较
                if (attribute.getName().trim().equals(key_value[0].trim()) && 
                    attribute.getText().trim().equals(key_value[1].trim())) {
                    System.out.println("找到匹配记录，准备删除");
                    find = true;
                    break;
                }
            }
