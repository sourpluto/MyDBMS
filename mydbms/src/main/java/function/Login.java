...
//            //处理分行输入的问题，就是读;号才停止;
                ...
                while(sql.lastIndexOf(";")!=sql.length()-1){
                    System.out.print("继续输入SQL语句（以分号结束）: ");
                    if (!input.hasNextLine()) {
                        System.out.println("输入流已关闭，程序退出");
                        return;
                    }
                ...
                // 增强quit检查
                if(sql.trim().equals("quit ENDOFSQL") || sql.trim().toLowerCase().equals("quit ENDOFSQL")) {
                    System.out.println("程序退出");
                    return;
                }

                List<List<String>> parameter_list = new ArrayList<>();
